...
class Navigator :
    ...
    def pubPose(self, event):
        ...
        p.pose.covariance[14] = 0.001
        
        print 'pose msg:', p
        self.pub_pose.publish(p)
        ####################################found out pose 22nya harus sama supaya errornya gak gedhe
        # Publish TF
    ...
    def pubLandmark(self, event):
        ...
        #~ p_t = PyKDL.Vector(-1.209, 0, 0.0)
        p_t = PyKDL.Vector(10.0, 5.0, 3.0)
        #~ p_t = PyKDL.Vector(-1.1, 3.7, 0.0)
        
        #~ rel_pose = r * (p_t - t)
        rel_pose = PyKDL.Vector(-1.1, 3.7, 0.0)
