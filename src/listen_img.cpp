...
float CALIBRATE_DEG_TO_PIXEL = (160.0/62.0); /// 1 frame, width x = 160px, 
...
void landmarkPublishtoEKF(int ii) 
{
		...
		static tf::TransformBroadcaster br;
		  ...
		  geometry_msgs::PoseWithCovarianceStamped p;
			...
			p.pose.pose.position.x = 0; //0 dari diri sendiri, jarak di frame di post di tf  (transform)
...
void imuPublishtoEKF(void)
{
	...
	tf::Quaternion q;
	//~ q.setRPY(/**msg->theta**/0, 0, global_robot_bearing);
...
void Callback_rgb(const sensor_msgs::Image::ConstPtr& img)
{
	...
	int queuenum = 1;
	int divcount = 0;
	queue_y[queuenum] = img->height/2;
	while (queuenum>0)
	{
		baristengah_y = queue_y[1];
		int dptlandmark = 0;
		...
		if ((dptlandmark<1) && (divcount<3))
		{
			divcount++;
			queuenum++;
			queue_y[queuenum] = baristengah_y/2;
			cout<<"antrikan y="<<queue_y[queuenum]<<"\n";
			queuenum++;
			queue_y[queuenum] = int(baristengah_y*3.0/4.0);
			cout<<"antrikan y="<<queue_y[queuenum]<<"\n";
		}
	}
}
