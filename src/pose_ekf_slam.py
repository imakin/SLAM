...
class PoseEkfSlam :
    def __init__(self, name, p_var, q_var):
        ...
        #dua yang ini bisa langsung dipakai di RViz
	...
	"""MEMBUAT SUBSCRIBER UNTUK SETIAp landmark yang didapat (1 landmark 1 subscriber)"""
    ...
    def setLandmark(self, req):
        self.lock.acquire()
        self.mapped_landmarks[req.topic_name] = self.number_of_landmarks
    ...
    def poseUpdate(self, pose_msg):
        ...
        # disini stamp isinya waktu yg dikirim dr topic pose_msg, ternyata yg dipake buat hitung t di fungsi-fungsi ekf t = (waktu skrng ~nilaistamp) - (waktu terakhir proses)
        if self.makePrediction(pose_msg.header.stamp):
            ...
            if pose_msg.header.frame_id != self.robot_frame_name:
                if pose_msg.header.frame_id in self.tf_cache:
                    ...
                    try:
                        ...
                        rospy.logerr('%s, Define TF for %s update! (function poseUpdate)', 
            ...
            if tf_done:
                ...
                self.applyUpdate(z, r, h, v, 10.0) # ini max distance 10 (d)
                ...
                if twist_msg.header.frame_id in self.tf_cache:
                    ...
                    try:
                        ...
                        self.tf_cache[twist_msg.header.frame_id] = [trans, rot]
                            ...
                            tf.ExtrapolationException,
                            tf.Exception):
                        rospy.logerr('%s, Define TF for %s update! (velocity Update)', 
                                     self.name, 
                                     twist_msg.header.frame_id)
            else:
    ...
    def landmarkUpdate(self, landmark_msg, topic_name):
        ...
        if self.makePrediction(landmark_msg.header.stamp):
            if topic_name in self.mapped_landmarks:
                if landmark_msg.header.frame_id in self.tf_cache:
                    ...
                    landmark_id = self.mapped_landmarks[topic_name]
                    ...
                    Or = PyKDL.Rotation.RPY(angle[0], angle[1], angle[2])   #ini rotation matrix A*rot = A' http://en.wikipedia.org/wiki/Rotation_%28mathematics%29  Three Dimention
                    ...
                    self.landmark_last_update[self.mapped_landmarks[topic_name]] = landmark_msg.header.stamp
                ...
                if topic_name in self.candidate_landmarks:
                    ...
                    if trans != None:
                        ...
                        if len(self.candidate_landmarks[topic_name]) > 4:
                            ...
                            if update:
                                self.mapped_landmarks[topic_name] = self.number_of_landmarks
    ...
    def getTF(self, origin, destination, stamp):
        try:
            ...
            return [trans, rot]
                ...
                tf.ExtrapolationException,
                tf.Exception):
            rospy.logerr('%s, Define TF for %s update! (function getTF)', 
                         self.name, 
                         destination)
            return [None, None]
    ...
    def makePrediction(self, now):
        if self.is_ekf_init and self.is_imu_init:
            ...
            #~ # test 5 agustus 2014 hilangin invalid time krn t aneh
            #~ self.lock.acquire()
            #~ self.prediction(u, t)
            #~ self.lock.release()
            #~ return True
            ...
            # original if kemungkinan untuk case operasi awal ( last prediction belum ada )
            if t > 0.0 and t < 1.0:
                # Make a prediction                
                self.lock.acquire()
                self.prediction(u, t)
            ...
            # same prediction
            #~ elif t > -1: #asli -0.1, program unexpected dpt waktu selisih negaif (waktu sekarang kurang dari waktu terakhir prediksi) 28 juli 2014
            elif t > -0.1: #asli -0.1, program unexpected dpt waktu selisih negaif (waktu sekarang kurang dari waktu terakhir prediksi) 28 juli 2014 diganti -1
                self._x_ = self.x
                self._P_ = self.P
                return True
            else:
                #~ rospy.logerr('Invalid time: %s', t)
                rospy.logerr('Invalid time: %s.... now: %s', t, now )
                return False
        else:
            return False
    ...
    def publishData(self, now):
        if self.is_ekf_init:
            ...
            # Pose
            self.odom.pose.pose.position.x = self.x[0] ##posisi
            ...
            # Twist
            self.odom.twist.twist.linear.x = self.x[3] ## twist di x3,4,5 = angular velocity
    ...
    def addLandmark(self, x, y, z, measured_covariance):
        ...
        #gak pakai jacobian, tdk ada covariance landmark-landmark
        ...
        R = PyKDL.Rotation.RPY(angle[0], angle[1], angle[2])  	#hasilnya matriks rotasi 3x3. buku Kompetensi Matematika Jilid 3A SMA hal 180
...
if __name__ == '__main__':
    try:
        ...
        #~ pose_3d_ekf = PoseEkfSlam(rospy.get_name(), 
                                  ...
                                  #~ [0.05, 0.05, 0.05])
        pose_3d_ekf = PoseEkfSlam(rospy.get_name(), 
                                  [0.001,0.001,0.001, 0.001,0.001,0.001], #xyzvxvyvz covariance P
                                  [0.001, 0.001, 0.001]) #matriks derau sistem
